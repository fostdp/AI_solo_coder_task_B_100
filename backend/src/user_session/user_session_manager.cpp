...
ShockVibration UserSessionManager::compute_shock_vibration(uint8_t damage_level,
                                                           double rock_mass_kg,
    ...
    double x_normalized = (impact_location_x + half_width) / vehicle_width_m;
    ...
    sv.audio_impact_intensity = static_cast<double>(damage_level) / 4.0;
...
RockAttackEvent UserSessionManager::generate_random_attack(const std::string& session_id,
    ...
    event.shock_effect = compute_shock_vibration(
        sim_result.damage_level,
        event.rock_mass_kg,
